...
var arcBandMain = new Kinetic.Shape({
    ...
    fill: listColors[segments.length],
    stroke: 'black',
    strokeWidth: 4
});
...
labelMain = new Kinetic.Text({
    x: SKETCH_CENTER_X-OUTER_CIRCLE_RADIUS-5 - mainSegmentLabel.length*8,
    y: SKETCH_CENTER_Y,
...
function funcDrawBezier(i){
    ...
    * (INNER_CIRCLE_RADIUS - INTERSTICE_BEZIER);
    ...
    bezierArr[i] = new Kinetic.Shape({
        ...
        fill: listColors[i],
        stroke: listColors[segments.length],
...
function funcDrawLabel(i){
    ...
    if (theta < minLimit | theta > maxLimit) {
        thetaLabel = theta + PI;
        offsetLabel = segments[i].label.length*8;
